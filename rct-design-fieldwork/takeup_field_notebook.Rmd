...
takeup.dict %>%
  summarise(
    n_p = n_distinct(person_key),
    n_k = n_distinct(KEY.individ),
    n_c = n_distinct(cluster_key)
  )
...
#          monitored = (!is.na(sms.treatment) & (sms.treatment == "sms.control" | (!is.na(sms.consent) & sms.consent))) | 
#                           individ.baseline.sample.pool,
...
census.data %>%
  summarize(n = sum(!is.na(sms.treatment)))


full_takeup_dict = c("data/takeup_survey_dict_wave1.csv") %>%  # Only needed for wave 1
  map_df(~ read_csv(.)) 
...
census.data %>%
  filter(wave == 1) %>%
  count(sms.treatment, true.monitored) 
...
census.data %>%
  filter(wave == 2) %>%
  count(sms.consent, sms.treatment, true.monitored)
...
census.data %>%
  count(!is.na(sms.treatment), !is.na(true.monitored))

census.data %>%
  count(monitored, sms.treatment)

census.data %>%
  write_csv("debug-census-data.csv")
...
census.data %>%
  filter(!is.na(sms.treatment)) %>%
  group_by(sms_control = sms.treatment == "sms.control") %>%
  count(true.monitored)
...
census.data %>%
  filter(wave == 1,
         ...
         sms.treatment == "sms.control" | sms.consent, # & between(sms.status.success, 4, 6)),
         is.na(sms.ctrl.backup) | !sms.ctrl.backup)  %>%
  count(true.monitored, sms.treatment)



# Wave 2 Endline Sample  - everyone monitored
...
endline_people_karim = bind_rows(wave1.endline.individuals, wave2.endline.individuals)

endline_people_karim
