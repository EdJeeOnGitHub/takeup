...
munge_freq_results = function(data) {
  data %>%
    mutate(
    ...
    mutate(
      assigned_treatment = factor(assigned_treatment, levels = c("bracelet", "calendar", "ink", "control")),
    ...
    mutate(treat_n = cur_group_id()) %>%
    ungroup() %>%
    mutate(
      colors = canva_pal(canva_palette_vibrant)(4)[treat_n]
    )
}
