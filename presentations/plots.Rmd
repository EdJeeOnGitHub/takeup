...
gpt_translation_df = gpt_translation_df %>%
    mutate(
...
gpt_endline_know_notknow_table_data %>%
    ...
    left_join(
        gpt_translation_df,
        by = c("category_sob_reason_short" = "gpt_name")
    )  %>%
    mutate(
        assigned.treatment = str_to_title(assigned.treatment),
...
freq_rf_results %>%
  ...
  labs(
    x = "",
    y = "",
    caption = "95% confidence intervals shown calculated using wild cluster bootstrap."
  )
...
```{r}
#| freq-rf-levels-discrete-dist,
#| fig.cap="Incentive Takeup Levels, Reduced Form Model - Discrete Distance",
#| fig.width = 8, fig.height = 6




fit_and_clean_discrete_rf = function(data, fit = NULL) {
  data = data %>%
    mutate(assigned_dist_group = dist.pot.group)
  if (is.null(fit)) {
    fit = feols(
      dewormed ~ 0 + assigned.treatment*assigned_dist_group,
      data = data,
      vcov = ~cluster.id
    )
  }
...
double_lasso_fit = feols(
  data = cov_analysis_data,
  dewormed ~ 0 + assigned.treatment*assigned_dist_group + .[lasso_cov_vars] | county,
  vcov = ~cluster.id
)
...
double_lasso_fit %>%
  tidy() %>%
  filter(
    term %in% lasso_cov_vars | str_detect(term, "TRUE")
  )

discrete_dl_fit = fit_and_clean_discrete_rf(cov_analysis_data, double_lasso_fit) %>%
  rename(assigned_treatment = assigned.treatment)
...
discrete_rf_fit = fit_and_clean_discrete_rf(analysis_data) %>%
  rename(assigned_treatment = assigned.treatment) 

plot_freq_rf_levels = function(data) {
  data %>%
  filter(fit_type == "prob") %>%
  filter(.width == 0.95) %>%
  filter(!is.na(assigned_treatment)) %>%
  mutate(
    alpha = case_when(
...
```{r}
#| freq-rf-tes-discrete-lasso-plot,
#| fig.cap="Incentive Takeup ATEs, Reduced Form Model - Discrete Distance, LASSO Covs",
#| fig.width = 8, fig.height = 6
...
```{r}
discrete_rf_fits = bind_rows(
  discrete_rf_fit %>% mutate(fit_type = "no covs, discrete distance"),
  discrete_dl_fit %>% mutate(fit_type = "double lasso + covs, discrete distance")
)

discrete_rf_fits %>%
  write_csv(
    "temp-data/discrete-rf-fits.csv"
  )

```

```{r}
#| freq-rf-levels-bar-plot,
#| fig.cap="Incentive Takeup Levels, Reduced Form Model",
#| fig.width = 8, fig.height = 6
...
freq_rf_levels %>%
  ...
  filter(assigned_dist_group != "Combined") %>%
  mutate(
    alpha = case_when(
      assigned_treatment == "Control" ~ 1,
      TRUE ~ 0.15
  ...
  scale_x_continuous(breaks = seq(0.0, 0.6, 0.1), limits = c(0.0, 0.6)) 
