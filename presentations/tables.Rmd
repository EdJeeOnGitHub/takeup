...
lhs_translation_df = tribble(
  ...
  "lhs: chv_visit", "Did a CHV visit you to discuss MDA?", "implementation"
) %>%
  mutate(
    lhs_no_prefix = str_remove(lhs, "lhs: ")
  ) %>%
    mutate(sample = factor(sample, levels = c("pretreat", "baseline_worm", "census", "takeup_sample", "implementation")))
...
panel_names = c(
      ...
      str_glue("Panel B: Baseline worm covariates, $N={format(N_baseline_worm, big.mark = ',')}$"),
      ...
      str_glue("Panel E: Implementation covariates, $N={format(N_endline, big.mark = ',')}$")
) 
