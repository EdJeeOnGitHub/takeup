    ...
    bind_rows(
      ...
      .data %>%
        filter(estimand == .estimand) %>%
          ...
          mutate(
            "{{ group_var }}" := "signal_minus_no_signal"
          )
    )
}
...
recode_control_mean = function(data) {
  data %>%
    mutate(
      treatment = fct_recode(treatment, "Control mean" = "Control", "Bracelet - Calendar" = "bracelet_minus_calendar", "Signal - No Signal" = "signal_minus_no_signal")
    )
}
...
obs_rvar_struct = read_rds(
  file.path(
    "temp-data",
    str_glue(
      "rvar_processed_dist_fit{params$fit_version}_belief_ates_{params$struct_models}_1-4.rds"
    )
  )
)
...
lvl_obs_rvar_struct = read_rds(
  file.path(
    "temp-data",
    str_glue(
      "rvar_processed_dist_fit{params$fit_version}_belief_probs_{params$struct_models}_1-4.rds"
    )
  )
) %>%
as.data.frame() %>%
...
```{r}
# #| fob-table, eval=FALSE

# I calculate this using rvars above now

# fob_wide_belief_tbl = wide_belief_df %>%
#   select(treatment, contains("_1")) %>%
#   kbl(
#     col.names = c(
#       # "Estimand", 
#       # "Treatment", 
...
wtp_input_df = tidy_param_df %>%
  ...
  mutate(name = if_else(variable == "hyper_wtp_mu", str_glue("Valuation difference (USD), mean"), name)) %>%
...
all_optim_df = read_csv(
  file.path(
    ...
    # "posterior-clean-summ-optim-w-static-control.csv"
...
filter(fix_type == "no-fix" | is.na(fix_type)) %>%
  ...
  mutate(
    ...
    c("Control", "Ink", "Calendar", "Signal value fixed at control 0.5km", "Signal value fixed at bracelet 0.5km", "Bracelet",  "No Visibility"))
  )  %>%
  mutate(
    mu_z = fct_recode(mu_z, 
...
```{r}
#| optim-tbl, eval=FALSE


optim_summ_tbl_diff_order =  optim_summ_input_df %>%
  filter(distance_constraint == 3500 | allocation_type == "experimental") %>%
  arrange(rep_type, mu_z, B_z) %>%
  mutate(
    mu_z = factor(
      mu_z, 
      levels = c("Control", "Bracelet", "Signal value fixed at bracelet 0.5km",  "No Visibility")
    )
  ) %>%
  arrange(rep_type, mu_z, B_z) %>%
  select(B_z, mu_z, n_pot, mean_demand, mean_dist)   %>%
...
optim_summ_tbl_diff_order %>%
  custom_save_latex_table("optim-summ-table-diff-order")


```

```{r}
#| fixing-optim-table, eval=FALSE
...
header = 
  list(
    "(1) Sample: Control" = 1,
    "(2) Sample: Control + Ink" = 1,
    "(3) Sample: Control + Ink" = 1
  )
...
pref_gift_fit_not_dewormed = analysis_data %>%
    ...
    ungroup() %>%
    select(cluster.id, gift_choice, assigned.treatment, dist.pot.group, county) %>%
    mutate(
      want_bracelet = gift_choice == "bracelet"
    )  %>%
    group_by(assigned.treatment, dist.pot.group)  %>%
    mutate(treatment = assigned.treatment) %>%
    fixest::feols(
      want_bracelet ~  treatment*dist.pot.group | county,
      cluster = ~cluster.id
    )
...
etable(
  pref_gift_fit_not_dewormed,
    ...
    fitstat = c("my", "n"),
    headers = list(
      "Bracelet  Preference" = 1
    ),
    depvar = FALSE,
    file = file.path(
      params$table_output_path, "bracelet-wtp-across-dist.tex"
    ),
    drop.section = "fixef",
    tex = TRUE, 
    postprocess.tex = tex_postprocessing,
...
pref_gift_fit_tbl = fixest::etable(
  ...
  depvar = FALSE,
  replace = TRUE,
  # title = "Preference for Bracelet Gift Across Distance and Condition",
  style.tex = fixest::style.tex("aer"),
  headers = pref_gift_fit_header,
  file = "presentations/tables/pref-gift-by-arm.tex"
  )
...
```{r}
#| ramsey-output, eval=FALSE

dynamic_optim_post_name = "posterior-optimal-distance-dynamic_vstar-lambda_0.15-externality_0.15-model_STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP-fit-version_86.csv"
static_optim_post_name = "posterior-optimal-distance-static_vstar-lambda_0.15-externality_0.15-model_STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP-fit-version_86.csv"
...
ramsey_optim_te_tbl_tex = optim_te_tbl %>%
  select(-vstar_type) %>%
  kbl(
    col.names = c(
      "Treatment",
...
ramsey_optim_te_tbl_tex

ramsey_optim_te_tbl_tex %>%
  custom_save_latex_table(
    "ramsey-optim-te-table"
  )
...
```{r}
#wide-ramsey, eval=FALSE
# wide_ramsey_optim_te_tbl_tex = optim_te_tbl %>%
#   pivot_wider(
...
# wide_ramsey_optim_te_tbl_tex

# wide_ramsey_optim_te_tbl_tex %>%
#   custom_save_latex_table(
#     "wide-ramsey-optim-te-table"
#   )
...
```{r}
#| robustness-struct-te-table, eval=TRUE
...
# Robustness Structural Model Individual Distance Cost, Community FP, Indiv Vis

```{r}1+1

ate_rvar_struct_indiv_dist_community_fp_indiv_vis = read_rds(
    file.path(
      "temp-data",
      "rvar_processed_dist_fit103_INDIV_incentive_tes_STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP_INDIV_DIST_COMMUNITY_FP_INDIV_VIS_1-2.rds"
    )
  )


ate_tbl_indiv_dist_indiv_fp_indiv_vis = ate_rvar_struct_indiv_dist_community_fp_indiv_vis %>%
  prep_robustness() 



ate_tbl_indiv_dist_indiv_fp_indiv_vis %>%
  create_robustness_tbl()  %>%
  custom_save_latex_table("indiv-dist-community-fp-indiv-vis-robust-struct-overall-te-table")
...
1+1
ate_rvar_struct_no_outliers = read_rds(
    file.path(
      "temp-data",
      "rvar_processed_dist_fit103_ates_STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP_NO_OUTLIERS_1-2.rds"
    )
  )

incentive_ate_df_no_outliers =  ate_rvar_struct_no_outliers %>%
  create_ate_table(
    .estimand = "overall",
    group_var = treatment
  ) %>%
...
incentive_ate_no_outliers_kbl = incentive_ate_df_no_outliers %>%
  select(-model, -estimand) %>%
  arrange(treatment) %>%
  select(-contains("rf_")) %>%
  kbl(
...
incentive_ate_no_outliers_kbl %>%
  custom_save_latex_table("struct-robustness-nooutliers-overall-te-table")
