...
lhs_translation_df = tribble(
  ...
  "lhs: fully_aware_externalities", "Understands externalities", "baseline_worm",
  "lhs: partially_aware_externalities", "Partially understands externalities", "baseline_worm",
  "lhs: female", "Female", "census",
  ...
  "lhs: n_per_cluster", "Number of individuals per community", "census",
...
comp_wide_indiv_bal_df = create_balance_input(
 ...
 select(all_of(col_diff_order)) %>%
  ...
  left_join(lhs_translation_df %>% select(lhs, clean_name, sample)) %>%
...
panel_names = c(
      str_glue("Panel A: Pretreat covariates, $N={format(N_pretreat, big.mark = ',')}$"), 
      str_glue("Panel B: Census covariates, $N={format(N_census,  big.mark = ',')}$"), 
      str_glue("Panel C: Baseline worm covariates, $N={format(N_baseline_worm, big.mark = ',')}$"),
      str_glue("Panel D: Takeup sample covariates, $N={format(N_takeup, big.mark = ',')}$")
) 
...
dist_comp_bal_tbl = dist_comp_df %>%
  left_join(lhs_translation_df %>% select(lhs, clean_name)) %>%
  mutate(lhs = clean_name) %>%
  select(-clean_name) %>%
  arrange(lhs == "Distance to PoT") %>%
  knitr::kable(
      format = "latex",
