...
optim_post_df  %>%
  pivot_wider(
    names_from = vstar_type, values_from = c(optimal_distance, pred_takeup)
  )   %>%
  group_by(treatment) %>%
  summarise(
    across(c(contains("dynamic"), contains('static')), mean)
  ) %>%
  mutate(
    across(contains("distance"), ~round(.x/1000, 2))
  ) %>%
  select(treatment, contains("distance"), contains("pred")) %>%
  mutate(treatment = fct_relevel(treatment, "control", "ink", "calendar", "bracelet"))  %>%
  arrange(treatment)
...
ate_tbl_indiv_dist_community_fp_indiv_vis = ate_rvar_struct_indiv_dist_community_fp_indiv_vis %>%
  create_cis(.width = params$width) %>%
  select(
    treatment, variable, value
  ) %>%
...
ate_tbl_indiv_dist_community_fp_indiv_vis %>%
  create_robustness_tbl()  %>%
