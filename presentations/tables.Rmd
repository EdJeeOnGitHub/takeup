...
lhs_translation_df = tribble(
  ...
  "lhs: announcement", "Was there an announcement about MDA in your community?", "implementation"
) %>%
  mutate(
...
panel_names = c(
      str_glue("Panel A: Takeup sample covariates, $N={format(N_takeup, big.mark = ',')}$"),
      str_glue("Panel B: Pretreat covariates, $N={format(N_pretreat, big.mark = ',')}$"), 
      str_glue("Panel C: Baseline worm covariates, $N={format(N_baseline_worm, big.mark = ',')}$"),
      # str_glue("Panel C: Census covariates, $N={format(N_census,  big.mark = ',')}$"), 
      str_glue("Panel D: Implementation covariates, $N={format(N_endline, big.mark = ',')}$")
) 
...
cts_dist_fits %>%
  ...
  left_join(
    cts_distance_pval_df %>%
      rename("Binned F-test $p$-value" = value),
      by = c("lhs")
  ) %>%
  knitr::kable(
      format = "latex",
      col.names = c(
        ...
        "F-test $p$-value, binned distance (750m)"
        ),
      escape = FALSE, 
      booktabs = TRUE,
      align = "lccc", 
...
```{r}
robustness_ate_rvar_df = read_rds(
      file.path(
        "temp-data",
        str_glue("rvar_processed_dist_fit96_ates_REDUCED_FORM_NO_RESTRICT_DIST_CTS_1-4.rds")
      ) 
  )

robustness_ate_rvar_df

robustness_ate_df =  robustness_ate_rvar_df %>%
  create_ate_table(
    .estimand = "overall",
    group_var = treatment
  ) 

robustness_ate_tbl = robustness_ate_df %>%
  mutate(treatment = factor(treatment, levels = 
    ate_tbl_levels
  )) %>%
  mutate(far_minus_close = far - close) %>%
...
robustness_kbl_df = robustness_ate_tbl %>% 
  arrange(treatment) %>%
  select(-estimand) %>%
  kbl(
    col.names = c(
