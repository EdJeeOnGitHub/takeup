...
\changed{Added \code{scripts/appendix/sigma-u-prior-posterior-plot.R}, which reads only the \(\sigma_u\) Stan variable, writes a prior/posterior comparison plot, and writes a summary CSV. The paper now explains that \(\sigma_u\) is disciplined by curvature in arm-by-distance take-up cells and the normal-sum signal-extraction restriction.}
...
\changed{Added \code{archive/code/scratch/stan-hmc-diagnostics.R}, a reusable diagnostics script that can load only selected Stan variables. The paper now reports max split \(\hat R\), minimum bulk/tail ESS, and divergence counts for the main structural fit and the appendix structural robustness fits.}
