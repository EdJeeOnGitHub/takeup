...
\changed{\code{balance-functions.R} now reports raw Control means from the relevant estimation sample instead of the omitted-county fixed-effect intercept. \code{balance.R} passes the underlying data into the balance helper so the same fix applies across main, appendix, attrition, monitoring-attrition, and SMS-enrollment paths.}
