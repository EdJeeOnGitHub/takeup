...
In this section, we show how we generate the closed form solution for $\Delta(w)$ using the 
Gaussian distribution - both the unbounded case used in the paper, and the bounded case for completeness since \cite{Benabou_laws_2011} place bounds on $v$.
\subsubsection{$\Delta(w(b,c))$ Derivation}
We want to find:
...
\end{align*}
Reintroducing the first fraction:
\begin{align*}
    \Delta(w(b,c)) &= \frac{
        -1
    }{
...
Focusing on the integral and recognising we have derived this above:
\begin{align*}
    \int^\infty_{-\infty} v \frac{1}{\sigma}\phi\left(\frac{w - v}{\sigma}\right) \phi(v) dv \\ &=
...
\subsubsection{Bounded $\Delta(w(b,c)$ Derivation}
...
\begin{align*}
    \Delta(w(b,c)) &= \frac{-1}{F_w(w)(1 - F_w(w))} \frac{1}{\Phi(\overline{v}) - \Phi(\underline{v})}  \\
    &\times\left(
...
That is, the convolution of two independent r.v.s. It can be shown that the RHS:
\begin{align*}
    RHS &= \frac{1}{2} \left(
        \Phi(z_h) - \Phi(z_l)
    \right)  \\
...
Using a change of variables on the above gives:
\begin{align*}
    \int^h_l \Phi(x)\frac{1}{\sigma} \phi(\frac{x - \mu}{\sigma})dx
\end{align*}
where $T$ stands for Owen's T.
