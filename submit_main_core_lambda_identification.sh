...
case "${PHASE}" in
  core)
    prepare_job=$(sbatch --parsable --export=ALL,STAGE=prepare \
      slurm_main_core_lambda.sh)
    reuse_job=$(sbatch --parsable --dependency=afterok:${prepare_job} \
      --export=ALL,STAGE=reuse_common slurm_main_core_lambda.sh)
    ...
    gate_job=$(sbatch --parsable --dependency=afterok:${reuse_job} --array=1 \
      --export=ALL,STAGE=mode slurm_main_core_lambda.sh)
    ...
    ;;
  recovery-generate)
    sbatch --export=ALL,STAGE=generate slurm_main_core_lambda_recovery.sh
    ;;
  recovery-fit-a)
    sbatch --array=1-375%${MAX_RECOVERY_JOBS} --export=ALL,STAGE=fit \
      slurm_main_core_lambda_recovery.sh
    ;;
  recovery-fit-b)
  ...
  recovery-finish)
    hmc_gq_job=$(sbatch --parsable --array=1-30 \
      --export=ALL,STAGE=hmc_gq slurm_main_core_lambda_recovery.sh)
    summary_job=$(sbatch --parsable --dependency=afterany:${hmc_gq_job} \
      --export=ALL,STAGE=summarize slurm_main_core_lambda_recovery.sh)
    assemble_job=$(sbatch --parsable --dependency=afterok:${summary_job} \
      --export=ALL,STAGE=assemble slurm_main_core_lambda_recovery.sh)
    printf '%-24s %s\n' hmc_gq "${hmc_gq_job}" \
