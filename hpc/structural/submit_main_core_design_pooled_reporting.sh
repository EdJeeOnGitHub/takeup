...
sample_job=$(sbatch --parsable --time=04:00:00 --array=1-4 \
...
gq_compile_job=$(sbatch --parsable --dependency="afterok:${sample_job}" --array=1 \
  ...
  hpc/structural/slurm_main_core_observability_gq.sh)
...
summary_job=$(sbatch --parsable --dependency="afterok:${gq_compile_job}:${gq_job}" \
  --export="ALL,ROOT=${OUTPUT_ROOT}" hpc/structural/slurm_main_core_report_distance_summary.sh)
printf '%-12s %s\n' sample "${sample_job}" gq_compile "${gq_compile_job}" \
