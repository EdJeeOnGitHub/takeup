...
case "${STAGE}" in
  generate)
    ...
    Rscript --no-save --no-restore --no-init-file \
      scripts/appendix/generate-main-core-lambda-recovery.R \
  ...
  summarize)
    Rscript --no-save --no-restore --no-init-file \
      scripts/appendix/summarize-main-core-lambda-recovery.R \
      "--manifest=${MANIFEST}" "--output-path=${OUTPUT_PATH}"
    ;;
  assemble)
    Rscript --no-save --no-restore --no-init-file \
      scripts/appendix/assemble-main-core-lambda-appendix.R \
