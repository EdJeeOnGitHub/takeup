...
- `scripts/structural/run-model.R`
...
- `hpc/appendix/slurm_beliefs_robustness.sh`
...
```bash
sbatch hpc/appendix/slurm_beliefs_robustness.sh 106
```
...
```bash
ITER=800 sbatch hpc/appendix/slurm_beliefs_robustness.sh 106
```
...
```bash
Rscript -e 'parse("scripts/structural/run-model.R"); parse("R/structural/legacy-utils.R")'
git diff --check
bash -n hpc/appendix/slurm_beliefs_robustness.sh
