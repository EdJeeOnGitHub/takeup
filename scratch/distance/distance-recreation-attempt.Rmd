...
# N.B. Ed's replacement converting from rgeos to sf
# pot.info %<>% 
...
pot.info %<>% 
  filter(!is.na(lon.verify), !is.na(lat.verify),
         !is.na(alt.pot.lon), !is.na(alt.pot.lat)) %>% 
  mutate(
    verify.dist = st_distance(
      # Convert alt coords to sf and project
      st_transform(
        st_as_sf(., coords = c("alt.pot.lon", "alt.pot.lat"), crs = wgs.84),
        kenya.proj4
      ),
      # Convert verified coords to sf and project
      st_transform(
        st_as_sf(., coords = c("lon.verify", "lat.verify"), crs = wgs.84),
        kenya.proj4
      ),
      by_element = TRUE  # distance row i -> row i
    ) %>%
      as.numeric()  # convert from 'units' to plain numeric if desired
  ) %>% 
  select(KEY, verify.dist) %>% 
  right_join(pot.info, by = "KEY")
...
rct.villages <- read_rds(here("data", "rct_target_villages_2.0.rds")) %>% 
  ...
  bind_rows(read_rds(here("data", "rct_target_villages_2.0-4.rds" )) %>% 
              mutate(new.village = TRUE))


all.villages <- rct.villages %>%
  bind_rows(
    anti_join(cluster.survey.data, ., c("cluster.id", "target.village.id"))
  ) %>%
  mutate(
  ...
  left_join(rct.village.codes,
            by = c("cluster.id" = "village.cluster.id",
                   "target.village_name" = "village_name")) %>%
  left_join(select(pot.info, cluster.id, matches("alt.pot.(lon|lat)"), location_type),
            by = "cluster.id") %>%
  group_by(is.na(village)) %>%
  # Add new village IDs for any records not found in rct.village.codes
  do({
  ...
  mutate(
    dist.group = {
      # Convert points to sf, transform to Kenya CRS for distance in meters
      coords_sf <- st_as_sf(.,
      ...
      # Pairwise distance matrix (NxN), convert to 'dist' object, then cluster
      dist_mat <- st_distance(coords_sf, coords_sf)
      dist_vec <- as.dist(dist_mat)
      hc <- hclust(dist_vec)
      cutree(hc, h = 1000)  # group points within 1 km
    }
  ) %>%
  group_by(cluster.id) %>%
  mutate(num.dist.groups = n_distinct(dist.group)) %>%
  ungroup() %>%
  left_join(cluster.wave.county.data, by = "cluster.id") %>%
  mutate(
    village.name.group = target.village_name %>%
      str_trim() %>%
      str_replace("\\s*[A-Z]\\d?$", "") %>%
      str_replace_all("\\s+", " ") %>%
      str_replace_all("'", "") %>%
      ...
      str_replace("\\s+[A-Z]\\d?$", "") %>%
      str_replace(
        regex("\\s+(village|upper|lower|group|east|west|north|south|rural|urban|township|central)$",
              ignore_case = TRUE),
        ""
      ) %>%
    ...
    # String distance clustering for the standardized village names
...
pre.census.data <- read_rds(here("data", "pre.census.processed.rds"))
```
...
hh.census.data %<>% 
  anti_join(plyr::ldply(
    paste0(
        here::here("data"),
        "/",
        c("takeup_returns_toremove.csv", 
...
hh.id.dict <- read_rds(here::here("data", "takeup_hh_id_dict.rds"))
...
person.id.dict <- read_rds(here("data","takeup_person_id_dict.rds" ))
...
write_rds(cluster.wave.county.data, "EDTEMP-takeup_cluster_wave_county_2.0.rds")
```


```{r cluster-review, include=FALSE}
load(
    here::here("data", "takeup_cluster_review.RData"))
```
...
pot.ranges <- pot.info %>%
  # 1) Convert to sf (points), specifying lon/lat columns & WGS84 CRS
  st_as_sf(coords = c("alt.pot.lon", "alt.pot.lat"), crs = wgs.84) %>%
  buffer.clusters(.width = 2500) %>%
  # 3) Transform back to WGS84 for final polygons
  st_transform(crs = wgs.84) %>%
  # 4) Convert geometry to POLYGON (if needed; might already be POLYGON)
  st_cast("POLYGON") %>%
  # 5) Tag each polygon row with an ID
  dplyr::mutate(poly_id = dplyr::row_number()) %>%
  # 6) “Tidy” the polygons by extracting vertex coords into a data frame
  (function(sf_polys) {
    # Extract coordinates
    coords_df <- sf::st_coordinates(sf_polys) %>%
      tibble::as_tibble() %>%
      dplyr::rename(long = X, lat = Y) %>%
      # Add columns for plotting or grouping, if desired
      dplyr::mutate(
        order = dplyr::row_number(),
        piece = L2,
        group = paste(L1, L2, sep = ".")
      )
    # Merge back with any attributes from the sf object (e.g., cluster.id)
    # Note L1 in st_coordinates is the polygon index (row), so we join on poly_id
    coords_with_attr <- dplyr::left_join(
      coords_df,
      sf_polys %>% 
        sf::st_drop_geometry() %>% 
        dplyr::select(poly_id, cluster.id),   # stuff we want 
      by = c("L1" = "poly_id")
    )
    coords_with_attr
  }) %>%
  dplyr::mutate(boundary.type = "pot") %>%
  dplyr::select(
    id = L1,
    long,
    lat,
...
get.survey.boundary <- function(.data, .boundary.type, .group.by = c("cluster.id")) {
  .data %>%
    # 1) Remove rows with missing lon/lat
    dplyr::filter(!is.na(lon), !is.na(lat)) %>%
    
    # 2) Group by the specified columns (default = cluster.id)
    dplyr::group_by(dplyr::across(dplyr::all_of(.group.by))) %>%
    
    # 3) For each group, compute a convex hull and return a per-vertex data frame
    dplyr::do({
      group_data <- .
      
      # Pull out the group’s cluster ID (or other grouping var) for naming
      # (Assuming ".group.by" includes "cluster.id")
      cluster_id_val <- group_data$cluster.id[1]

      # Convert points to sf and combine them into a single hull
      hull_sf <-
        group_data %>%
        sf::st_as_sf(coords = c("lon", "lat"), crs = wgs.84) %>%
        
        # Summarize to a single multi-point geometry, then get the hull
        dplyr::summarize(geometry = sf::st_union(geometry)) %>%
        sf::st_convex_hull() %>%
        
        # Project to a planar CRS for buffer if needed
        sf::st_transform(kenya.proj4) %>%
        # gBuffer(..., width=0) often used to clean geometry in sp,
        # st_buffer(dist=0) does the same "clean" in sf
        sf::st_buffer(dist = 0) %>%
        
        # Return to WGS84
        sf::st_transform(wgs.84)

      # Convert polygon geometry to a vertex table
      coords_df <-
        hull_sf %>%
        sf::st_cast("POLYGON") %>%
        sf::st_coordinates() %>%
        ...
        dplyr::mutate(
          order = dplyr::row_number(),
          piece = L2,                 # ring index
          # Combine the geometry indices with the cluster ID to form a unique group
          group = paste(L1, L2, cluster_id_val, sep = "-"),
          hole = FALSE               # if you want an explicit 'hole' column
        )
    ...
    dplyr::ungroup() %>%
    # 4) Add boundary type
    dplyr::mutate(boundary.type = .boundary.type)
}
...
identify.closest.cluster <- function(.data, data.coords.formula = ~ lon + lat, key.variable = "KEY") {
  # Extract the names of the coordinate columns from the formula,
  # e.g. coords = c("lon", "lat")
  coords <- all.vars(data.coords.formula)
  
  .data %>%
    # Keep only rows where the coordinate columns are not NA
    filter(rowSums(is.na(model.frame(data.coords.formula, data = ., na.action = NULL))) == 0) %>%
    # Wrap in an anonymous function so we can re-use .located.data
    (function(.located.data) {
      # Convert the input data to sf, project, then measure distances to known villages
      dist.mat <- 
        st_as_sf(.located.data, coords = coords, crs = wgs.84) %>%
        st_transform(kenya.proj4) %>%
        st_distance(
        ...
        units::drop_units() %>%
        as.matrix() # Convert from a units-based distance object to a regular matrix
      
      # dist.mat now has dimensions: nrow(.located.data) x nrow(known.village.locations)
      # Each column corresponds to a single known.village.location;
      # each row corresponds to one of your .located.data points.



      # Keep the original code that “applies” over columns
      plyr::adply(dist.mat, 1, function(dist.row) {
        # dist.col = distances from ALL .located.data points to a SINGLE known village
        in.range.dist <- dist.row[dist.row <= 2000]
        ...
        if (length(in.range.dist) == 0) {
          ...
          tibble(
            ...
            closest.village  = known.village.locations$target.village.id[which(dist.row <= min.dist)],
            closest.cluster  = known.village.locations$cluster.id[which(dist.row <= min.dist)]
          ) %>%
            distinct(closest.cluster, .keep_all = TRUE)
        }
        
      }) %>%
        # Bind those distance results back to the original data
...
library(furrr)
plan(multicore)  # or plan(multicore) on Unix-like systems



identify.closest.cluster <- function(.data,
                                     data.coords.formula = ~ lon + lat,
                                     key.variable = "KEY") {
  # 1. Figure out which columns hold longitude/latitude
  coords <- all.vars(data.coords.formula)
  
  # 2. Filter out rows missing those coordinates
  .located.data <- .data %>%
    dplyr::filter(rowSums(is.na(model.frame(data.coords.formula, data = ., na.action = NULL))) == 0)
  
  # 3. Build a distance matrix (M x N):
  #    - M rows for known villages
  #    - N columns for each row in .located.data
  #    => Each column corresponds to one .located.data observation
  dist.mat <- 
    known.village.locations %>%
      st_as_sf(coords = c("target.lon", "target.lat"), crs = wgs.84) %>%
      st_transform(kenya.proj4) %>%
    sf::st_distance(
      .located.data %>%
        st_as_sf(coords = coords, crs = wgs.84) %>%
        st_transform(kenya.proj4)
    ) %>%
    units::drop_units() %>%
    as.matrix()
  
  # 4. Parallel column-wise iteration using furrr::future_map_dfr
  #    - seq_len(ncol(dist.mat)) iterates columns
  #    - Each column "j" corresponds to one row in .located.data
  dist_results <- furrr::future_map_dfr(
    .x = seq_len(ncol(dist.mat)),
    .f = function(j) {
      # Distances from all known villages to the j-th .located.data row
      dist.col <- dist.mat[, j]
      
      # Subset any <= 2000m
      in.range.dist <- dist.col[dist.col <= 2000]
      
      # Original data row (to replicate in the output)
      row_data <- .located.data[j, ]
      
      # If none are within 2km, return NA
      if (length(in.range.dist) == 0) {
        tibble::tibble(
          !! key.variable := row_data[[key.variable]],
          min.dist        = NA_real_,
          closest.cluster = NA
        )
      } else {
        # Find the minimum of those in range
        min_d <- min(in.range.dist)
        
        # Get all clusters at the min distance
        out <- tibble::tibble(
          min.dist         = min_d,
          closest.village  = known.village.locations$target.village.id[dist.col <= min_d],
          closest.cluster  = known.village.locations$cluster.id[dist.col <= min_d]
        ) %>%
          # If multiple clusters share the same min distance, we might get duplicates
          dplyr::distinct(closest.cluster, .keep_all = TRUE)
        
        # Replicate the KEY (or other fields) for each row if needed
        out[[key.variable]] <- row_data[[key.variable]]
  ...
  # 5. The final result is a data frame combining all columns (observations).
  #    Each row corresponds to the j-th .located.data row, possibly repeated
  #    if multiple clusters share the min distance
  dist_results %>%
    # If you only want these columns, you can select them explicitly:
    dplyr::select(dplyr::all_of(c(key.variable, "min.dist", "closest.cluster")))
}
...
```{r corrections}
cluster.954.to.drop <- read_rds(
    here::here("data", "cluster_954_to_drop.rds"))
