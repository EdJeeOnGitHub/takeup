...
create_comparisons = function(fit) {
    ...
    close_far_comp = hypotheses(
        close_far_tes,
        hypothesis = close_far_hypotheses,
        type = "response"
    ) %>%
        mutate(
    ...
    close_far_comp$contrast = names(close_far_hypotheses)
    ...
    if (cts_distance) {
        ...
        rep_distances = analysis_data %>%
            group_by(assigned_dist_group) %>%
            summarise(
                mean_distance = mean(standard_cluster.dist.to.pot)
            ) %>%
            mutate(assigned_dist_group = factor(assigned_dist_group, levels = c("close", "far"))) %>%
            arrange(assigned_dist_group) %>%
            pull()

        cm_close_far = predictions(
            fit,
            variables = list(assigned_treatment = "control"),
            newdata = datagrid(standard_cluster.dist.to.pot = rep_distances)
        )  
        cm_close_far_string = "b2 - b1 = 0"
        cm_dist_order = c('close', 'far')
        ...
        cm_close_far_string = cm_close_far  %>%
            as_tibble() %>%
            mutate(
                b_val = paste0("b", 1:n())
            ) %>%
            ...
            arrange(assigned_dist_group) %>%
            summarise(
                b_str = paste0(b_val[assigned_dist_group == "far"], " - ", b_val[assigned_dist_group == "close"], " = 0")
            ) %>%
            pull(b_str)
        cm_dist_order = cm_close_far$assigned_dist_group
    ...
    cm_close_far$assigned_dist_group = cm_dist_order
