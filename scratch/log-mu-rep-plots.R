...
sim_int <- expand.grid(
  w = seq(lb - bandwidth, ub + bandwidth, 0.1),
  u_sd = seq(1, 2.0, 0.05),
  mu = seq(0.1, 1, 0.05)
) %>% 
  filter(u_sd == 0 | mu == 1) %>% 
...
sim_int_bounded = expand.grid(
  ...
  rowwise() %>% 
  mutate(
    mu_delta = mu * calculate_delta_bounded(w, sqrt(1 + u_sd^2), u_sd, c(lb, ub))
  ) 
...
sim_int_bounded  %>%
  ...
  # filter(!any(mu_delta > ub + abs(lb) + bandwidth)) %>%
  # filter(!any(mu_delta > ub)) %>%
  filter(u_sd == 1) %>%
  filter(abs(w) < 3) %>%
  filter(!any(mu_delta > 100)) %>%
  filter(mu == 1) %>%
  ggplot(aes(
    x = w, 
    y = mu_delta, 
...
sim_int %>%
  ...
  ggplot(aes(
    x = w, 
    y = value, 
    group = interaction(u_sd, mu)
  )) +
  geom_line()


comp_sim_ints = bind_rows(
  sim_int %>% mutate(type = "unbounded") %>% 
    filter(delta_type == "mu_delta") %>%
    rename(mu_delta = value), 
  sim_int_bounded %>% mutate(type = "bounded", delta_type = "mu_delta")
)

comp_sim_ints %>%
  filter(delta_type == "mu_delta") %>%
  mutate(type = factor(type, levels = c("unbounded", "bounded"))) %>%
  # filter(u_sd == 0) %>%
  filter(mu == 1) %>%
  ggplot(aes(
    x = w, 
...
fit_version <- c("test-no-sign-restrict", 66)
...
output_basepath = str_glue("temp-data/output_dist_fit{fit_version}")[1]
...
dist_fit_data <- fit_version %>% 
  ...
  group_by(model, fit_type, model_type, version) %>% 
  ungroup()

dist_fit_data = dist_fit_data %>%
  mutate(
    model = if_else(
      version == "test-no-sign-restrict",
  ...
  mutate(model_name = case_when(
    fct_match(model, "STRUCTURAL_LINEAR_U_SHOCKS") ~ "Original Structural Model", 
    fct_match(model, "STRUCTURAL_LINEAR_U_SHOCKS_LOG_MU_REP") ~ "Log Mu Structural Model", 
    fct_match(model, "REDUCED_FORM_NO_RESTRICT") ~ "Reduced Form Model" 
  ))
...
roc_df = dist_fit_data %>% 
  filter(fct_match(fit_type, "fit"), fct_match(model, c("STRUCTURAL_LINEAR_U_SHOCKS_LOG_MU_REP"))) %>%
  transmute(
    model_name,
...
dist_fit_data %>% 
  filter(
    fct_match(fit_type, "fit"), 
    fct_match(model, c("STRUCTURAL_LINEAR_U_SHOCKS", "STRUCTURAL_LINEAR_U_SHOCKS_LOG_MU_REP"))) %>%
