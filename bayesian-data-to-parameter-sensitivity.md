...
```sh
Rscript generate_bayesian_sensitivity_quantities.R 104 \
...
```sh
Rscript bayesian_sensitivity.R 104 \
  --model=STRUCTURAL_LINEAR_U_SHOCKS_PHAT_MU_REP \
  --derived
```
...
- `generate_bayesian_sensitivity_quantities.R`, which reconstructs the
  original Stan data and evaluates the likelihood blocks at the saved draws;
- `bayesian_sensitivity.R`, which calculates and formats the sensitivity
  matrix.
