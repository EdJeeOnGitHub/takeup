...
baseline_csv <- policy_option_value(
  args, "--baseline-csv",
  "temp-data/policy-cluster-bootstrap/policy-cluster-bootstrap-summary.csv"
)
...
model_order <- c(
  "baseline-cluster-bootstrap", "correct-observability",
  "second-order-observability", "grouped-lambda", "arm-lambda",
  "student-t5", "cluster-shock"
)
model_paths <- file.path(input_root, model_order[-1L], "policy-model-summary.csv")
...
baseline$model_id <- "baseline-cluster-bootstrap"
baseline$model_label <- "Baseline model (cluster bootstrap)"
baseline$scenario_label[baseline$scenario_id == 0L] <- "Experimental allocation"
baseline$draws <- baseline$replicates
baseline$target_infeasible_share <- ifelse(baseline$scenario == "suppress-reputation", 1, 0)
...
contrast_paths <- file.path(
  input_root, model_order[-1L], "policy-model-contrast-summary.csv"
)
...
baseline_replicates <- read.csv(
  "temp-data/policy-cluster-bootstrap/policy-cluster-bootstrap-replicates.csv",
...
baseline_contrast <- do.call(rbind, lapply(list(
  ...
  data.frame(
    model_id = "baseline-cluster-bootstrap",
    model_label = "Baseline model (cluster bootstrap)",
